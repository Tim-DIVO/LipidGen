...
######################################
# TEMPLATES: EXACT .min / .nvt files #
######################################

MIN_TEMPLATE = r"""# ----------------- Init Section -----------------
# ----------------- Init Section -----------------
# system.in.init
...
#############################################
# HELPER FUNCTIONS TO DO LINE-BY-LINE REPLACE
#############################################

def customize_min_text(original_text, base_name, data_input):
    """
    1) read_data bilayer.data -> read_data {data_input}
    2) 'traj_min.lammpstrj'    -> 'traj_min_{base_name}.lammpstrj'
    3) 'system_after_min.data' -> 'system_after_min_{base_name}.data'
    """
    lines = []
    for line in original_text.splitlines():
        strip_line = line.strip()
        # 1) read_data
        if strip_line.startswith("read_data bilayer.data"):
            line = line.replace("bilayer.data", data_input)
        # 2) "traj_min.lammpstrj"
        if "traj_min.lammpstrj" in line:
            line = line.replace("traj_min.lammpstrj", f"traj_min_{base_name}.lammpstrj")
        # 3) "system_after_min.data"
        if "system_after_min.data" in line:
            line = line.replace("system_after_min.data", f"system_after_min_{base_name}.data")
        lines.append(line)
    return "\n".join(lines) + "\n"

def customize_nvt_text(original_text, base_name):
    """
    1) read_data system_after_min.data -> read_data system_after_min_{base_name}.data
    2) full_traj_adapt.lammpstrj       -> full_traj_adapt_{base_name}.lammpstrj
    3) global_adapt.out                -> global_adapt_{base_name}.out
    4) rdf_adapt.out                   -> rdf_adapt_{base_name}.out
    5) msd_adapt.out                   -> msd_adapt_{base_name}.out
    6) system_after_nvt_adapt.data     -> system_after_nvt_adapt_{base_name}.data
    """
    lines = []
    for line in original_text.splitlines():
        # read_data
        if "read_data system_after_min.data" in line:
            line = line.replace("read_data system_after_min.data",
                                f"read_data system_after_min_{base_name}.data")
        # other output files
        if "full_traj_adapt.lammpstrj" in line:
            line = line.replace("full_traj_adapt.lammpstrj",
                                f"full_traj_adapt_{base_name}.lammpstrj")
        if "global_adapt.out" in line:
            line = line.replace("global_adapt.out", f"global_adapt_{base_name}.out")
        if "rdf_adapt.out" in line:
            line = line.replace("rdf_adapt.out", f"rdf_adapt_{base_name}.out")
        if "msd_adapt.out" in line:
            line = line.replace("msd_adapt.out", f"msd_adapt_{base_name}.out")
        if "write_data system_after_nvt_adapt.data" in line:
            line = line.replace("system_after_nvt_adapt.data",
                                f"system_after_nvt_adapt_{base_name}.data")
        lines.append(line)
    return "\n".join(lines) + "\n"

#############################################
# MAIN FUNCTION: GENERATE .min & .nvt FILES
#############################################

def generate_min_nvt(data_files):
    """
    data_files: list of 'bilayer_XXX.data' filenames, each unique.
    For each data file 'bilayer_XXX.data', we create:
      - min_bilayer_XXX.in
      - nvt_bilayer_XXX.in
    that reference custom-named output so there's no collision.
    """
    for df in data_files:
        # e.g. "bilayer_chol0.20_equal.data"
        base = os.path.splitext(os.path.basename(df))[0]
        # e.g. "bilayer_chol0.20_equal"

        min_filename = f"min_{base}.in"
        nvt_filename = f"nvt_{base}.in"

        # (A) build .min text
        min_text = customize_min_text(MIN_TEMPLATE, base, df)

        # (B) build .nvt text
        nvt_text = customize_nvt_text(NVT_TEMPLATE, base)

        # write them
        with open(min_filename, "w") as f:
            f.write(min_text)
        with open(nvt_filename, "w") as f:
            f.write(nvt_text)

        print(f"Created {min_filename}, {nvt_filename} for data '{df}'")

if __name__ == "__main__":
    # EXAMPLE usage:
    # Suppose we have 9 data files from the assembly stage:
    # You can edit or read from sys.argv, etc.
    example_data_files = [
        "bilayer_chol0.30_equal.data",
        "bilayer_chol0.30_u70.data",
        "bilayer_chol0.30_u30.data",
        "bilayer_chol0.40_equal.data",
        "bilayer_chol0.40_u70.data",
        "bilayer_chol0.40_u30.data",
        "bilayer_chol0.50_equal.data",
        "bilayer_chol0.50_u70.data",
        "bilayer_chol0.50_u30.data",
    ]

    generate_min_nvt(example_data_files)
