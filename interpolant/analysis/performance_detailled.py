...
root_path   = ".."
ckpt_path   = f"../lipidgen_best_{model_name}.pt"
...
def main():
    ...
    val_folds = ['configf_1200', 'configf_3712', 'configf_2704', 'configf_925', 'configf_3265']
    ...
    sF, eF = eR,                     eR + C_real_FG * N_PHASE_MAPS * N_FG_CLASSES_PER_MAP
    ...
    for i, folder_name in enumerate(val_folds):
        ...
        for step in range(num_predictions):
            ...
            # CHANGED: extract & unnormalize ALM coeffs for pred
            # radial:
            # 1) Radial  
            radial_real = torch.from_numpy(pred[sR:eR].cpu().numpy()).reshape(1, C_pos_RADIAL, 2)  
            radial_cplx_norm = real_to_complex_channel_stack(radial_real)  
            radial_coeffs_orig = unnorm_healpix_alms_final(radial_cplx_norm, all_stats["radial"])[0,:,0]  
            rad_seq.append(radial_coeffs_orig)  
    
            # 2) Phase FG  
            fg_real_flat   = torch.from_numpy(pred[sF:eF].cpu().numpy())  
            fg_real_coeffs = fg_real_flat.reshape(1, C_pos_FG, 2)  
            fg_cplx_norm   = real_to_complex_channel_stack(fg_real_coeffs)  
            fg_coeffs_orig = unnorm_healpix_alms_final(fg_cplx_norm, all_stats["phase_fg"])[0]  
            fg_seq.append(fg_coeffs_orig)  
    
            # 3) Phase BG  
            bg_real_flat   = torch.from_numpy(pred[sB:eB].cpu().numpy())  
            bg_real_coeffs = bg_real_flat.reshape(1, C_pos_BG, 2)  
            bg_cplx_norm   = real_to_complex_channel_stack(bg_real_coeffs)  
            bg_coeffs_orig = unnorm_healpix_alms_final(bg_cplx_norm, all_stats["phase_bg"])[0]  
            bg_seq.append(bg_coeffs_orig) 
        ...
        outdir = f"{folder_name}_{model_name}_alms"
        os.makedirs(outdir, exist_ok=True)
        np.savez_compressed(
            os.path.join(outdir, "trajectory_alms.npz"),
            radial   = np.stack(rad_seq, axis=0),   # (steps, Cpos_R)
            phase_fg = np.stack(fg_seq,  axis=0),   # (steps, Cpos_F, M_fg)
            phase_bg = np.stack(bg_seq,  axis=0)    # (steps, Cpos_B, M_bg)
        )
        print(f"  · Saved merged ALMs to {outdir}/trajectory_alms.npz")
