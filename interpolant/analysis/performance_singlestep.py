...
MAX_PREDICTION_HORIZON = 1  # How many frames to predict into the future


stats_file  = "../sh_norm_stats_correct.pkl"
val_file    = "../test_folders_final.txt"
root_path   = ".."
ckpt_path   = f"lipidgen_best_big_{model_name}.pt"
...
def main():
    ...
    for i, folder_name in enumerate(val_folds):
        ...
        T = traj["radial"].shape[0]
         ...
         # CHANGED: init lists to collect unnormalized ALMs per step
        rad_seq, fg_seq, bg_seq = [], [], []
        ...
        for step in range(num_predictions):
            ...
            taus = torch.linspace(0, 1.0, SDE_STEPS, device=device).unsqueeze(-1).unsqueeze(-1)
                        ...
                        # initialize per‐step buffers once (before tau-loop)
            b_hat_seq = []
            b_true_seq = []
            eta_hat_seq = []
            eta_true_seq = []

            # if you want three attention snapshots at τ≈0.1/0.5/0.9
            attn_snapshots = {}
            target_taus = { int(0.1*SDE_STEPS): 't10',
                            int(0.5*SDE_STEPS): 't50',
                            int(0.9*SDE_STEPS): 't90' }
            ...
            with torch.no_grad():
                for k, tau in enumerate(taus):
                    ...
                    # forward pass: also grab attention if your model exposes it
                    # (here we assume `net` populates net.latest_attn as a dict of layers→[heads→(seq×seq)] )
                    b_hat, eta_hat = net(x_t=x_t.unsqueeze(1),
                                         x_tau=x_tau3d, cfg=cond, tau=tau,
                                         return_attn_weights=True)
                    # compute the *true* drift at this τ
                    # b* = (x_{t+1}-x_t) + γ̇(τ)·z_tau
                    gamma_dot_val = gamma_dot(tau).unsqueeze(-1)  # shape to match z_tau
                    b_true = (seq[:, next_frame_idx] - x_t).cpu() + (gamma_dot_val * z_tau).squeeze(1).cpu()
                    eta_true = z_tau.squeeze(1).cpu()

                    # stash for every k
                    b_hat_seq .append(b_hat.squeeze(1).cpu())
                    b_true_seq.append(b_true)
                    eta_hat_seq .append(eta_hat.squeeze(1).cpu())
                    eta_true_seq.append(eta_true)

                    # if this k matches one of our target τ, save attention maps
                    if k in target_taus:
                        attn_snapshots[target_taus[k]] = {
                            layer: W.detach().cpu().numpy()
                            for layer, W in net.latest_attn.items()
                        }
            ...
            # ─┤ after the **step** loop ├───────────────────────────────────────
        # now save your full b/η trajectories
        outdir = f"{folder_name}_{model_name}_b_eta"
        os.makedirs(outdir, exist_ok=True)
        np.savez_compressed(
            os.path.join(outdir, "drift_noise_trajectories.npz"),
            b_hat_seq  = np.stack([t.numpy() for t in b_hat_seq],  axis=0),
            b_true_seq = np.stack([t.numpy() for t in b_true_seq], axis=0),
            eta_hat_seq= np.stack([t.numpy() for t in eta_hat_seq],axis=0),
            eta_true_seq=np.stack([t.numpy() for t in eta_true_seq],axis=0),
        )
        # and dump your attention snapshots:
        
        with open(os.path.join(outdir, "attn_snapshots.pkl"), "wb") as fp:
            pickle.dump(attn_snapshots, fp)

        print(f"  · Saved drift/noise & attention to {outdir}/")
